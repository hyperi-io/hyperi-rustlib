...
pub fn generate_dockerfile(contract: &DeploymentContract) -> String {
    ...
    // Build the apt-get RUN block dynamically from native_deps
    let apt_block = build_apt_block(&contract.native_deps);

    format!(
        r#"# Generated by hyperi-rustlib deployment module
# Do not edit manually — regenerate from DeploymentContract
#
# License:   FSL-1.1-ALv2
# Copyright: (c) 2026 HYPERI PTY LIMITED
...
/// Build the apt-get RUN block from native deps contract.
///
...
/// All packages from default repos and custom repos are installed in a single
/// `apt-get install` call.
fn build_apt_block(deps: &super::native_deps::NativeDepsContract) -> String {
